...
function generateSalesOrder(customers, products, date, index) {
    ...
    if (products.length > 0) {
        ...
        for (const product of products) {
            ...
            let buyProbability = calculateBuyProbability(product.sale_price, amountToSpend, product.weight); // Base probability based on price
            function calculateBuyProbability(price, budget, weight, alpha = 0.7, beta = 0.3, k = 10, t = 0.5) {
                weight = Math.max(0, Math.min(1, weight));

                const affordability = Math.max(0, Math.min(1, 1 - (price / budget)));
                const score = (alpha * affordability) + (beta * weight);
