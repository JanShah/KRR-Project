function generatePurchaseOrders(supplierList, inventoryList, categories, date, funds, index) {
    const capacityLeft = 100 - getUsedVolume(inventoryList)
    if(capacityLeft < 5) {
        console.log(`Warehouse has ${capacityLeft.toFixed(2)}% remaining, no more purchase orders should be generated.`);
        return [];
    }
    console.log(`Warehouse has ${capacityLeft.toFixed(2)}% remaining`);
