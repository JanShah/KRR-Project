...
import "./runAdviserModule.js";
import "./shortDate.js";
import "./getWorkAction.js";
const { shortDate, getWorkAction, runAdviserModule, adviserModule } = globalThis._sharedLogic;
...
export default function messageHandler(worker, data) {
    ...
    switch (data.type) {
        case "settings":
            ...
            document.getElementById('packagingLeadTimeInput').value = settings.packagingLeadTime
            break;
        case "message":
        ...
        case "setup":
            ...
            if (data.success) {
                let activePage = getWorkAction();
                //post message AFTER db setup
                worker.postMessage({ "action": "getSettings" })
                if (activePage)
                    worker.postMessage({ "action": activePage });
                worker.postMessage({ "action": "complete" });
        ...
        case "completed":
            ...
            break;
        case "runAdviser":
            runAdviserModule(worker);
            break;
        case "adviserModule":
            adviserModule(worker, data.data);
            break;
        case "customers":
...
function createSummaryCharts(data) {
    ...
    c.draw()
    const salesOrders = getPeriodicDataAndLabels(data, "salesOrdersShipped", 7)
    const cancelledOrders = getPeriodicData(getDataColumn(data, "salesCancelled"), 7)
    const backOrders = getPeriodicData(getDataColumn(data, "delayedOrders"), 7)
    const packageShortage = getPeriodicData(getDataColumn(data, "packageShortage"), 7)
    ...
    const newCustomers = getPeriodicDataAndLabels(data, "customersGained", 7)
    const inactiveCustomers = getPeriodicData(getDataColumn(data, "customersLost"), 7)
    const dailyRating = getPeriodicDataAndLabels(data, "rating", 1);

    createComparisonChart(7, [
        createDataset("Average Rating", dailyRating.values, "rgba(75, 228, 36, 0.8)", 3),
    ], dailyRating.labels, "Average Customer Rating", "Score", "line");
    ...
    createComparisonChart(4, [
        createDataset("Shipped", salesOrders.values, "rgba(54, 162, 235, 0.6)"),
        createDataset("Cancelled", cancelledOrders, "rgba(255, 99, 132, 0.6)"),
        createDataset("Inventory Delayed", backOrders, "rgba(255, 206, 70, 0.6)"),
        createDataset("Packaging Delayed", packageShortage, "rgba(100, 206, 70, 0.6)")

    ], salesOrders.labels, "Shipped Orders", "Count");
    ...
    function createComparisonChart(id, datasets, labels, title, yLabel, type = 'bar') {
...
function createDataset(label, data, backgroundColor, borderWidth = 1, border = 'black') {
    return {
        label: label,
        data: data,
