function createPurchaseOrder(products, category, date, funds, index) {
    ...
    products.forEach(p => {
        const qty = Math.max(1,randInt(1, 5));
        ...
        if (!canBuy) {
            // console.log("Cannot buy from supplier", supplier);
            return null;
        }
        if (willBuy) {
            if ((credit - lineTotal) < 0) {
                // console.log("Credit limit will be exceeded for supplier", supplier, p, qty, lineTotal)
            }
            if (credit + funds - lineTotal < 0) {
                // console.log("Funds will be exceeded for supplier", supplier)
                return null;
            }
