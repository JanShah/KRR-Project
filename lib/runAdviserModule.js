...
function runAdviserModule(localWorker) {
    ...
    if (DEBUGGING) console.log("Adviser Module Started");
}

function demandForecast(orders, horizon = 12, alpha = 1.4, varianceFloor = 0, randomNoise = 0.1) {

    if (DEBUGGING) console.log("WEEKS OF COVER: ", horizon)
...
function adviserModule(worker, { prior, boxes }) {
    ...
    const maxSpace = Number(document.getElementById("maxSpace").value) / 10 || 0.10
    const [analysedOrder, orderData] = analyseProposedOrder(proposedOrder, boxObject, availableBudget, maxSpace)
    createDataTableOfSuggestions(orderData);
    ...
    analysedOrder.budget = availableBudget
    analysedOrder.maxSpace = maxSpace * 10
    worker.postMessage({ action: "pre-order", data: analysedOrder });
    if (DEBUGGING) console.log(proposedOrder, analysedOrder)
}
...
function proposePurchaseOrder(noOfWeeks, demandForecastData, currentBoxes) {
    ...
    if (DEBUGGING) console.log(roundedOrders);
    return roundedOrders;
}


function analyseProposedOrder(proposedOrders, boxes, budget = 500, maxSpace = 0.1) {
    const availableBudget = budget;
    const totalSpace = 450000000000;
    const availableSpace = totalSpace * maxSpace; // 10% of 150m3
    ...
    if (DEBUGGING) console.log("Constrained Orders:", constrainedOrders);
    ...
    const orderData = constrainedOrders.map(order => {
        ...
        return {
            ...
            "Space Used %": ((totalSpaceUsage * 10) / totalSpace).toFixed(2)
        };
    });
    ...
    return [constrainedOrders, orderData];
}

function createDataTableOfSuggestions(orderData) {
    const lastRow = orderData.reduce((acc, cur) => {
    ...
    table.append(tfoot)
    if (DEBUGGING) console.table(orderData);
    ...
    document.getElementById("useLastAdviser").checked = true;
...
function createTableFromObjectArray(data) {
    ...
    if (!Array.isArray(data) || data.length === 0) {
        if (DEBUGGING) console.error('Invalid data. Expected a non-empty array.');
        return;
    }
...
function createBayesianChart(futureDemandForecast, boxes) {
    ...
    products.forEach(product => {
        ...
        btn.addEventListener('click', showChart.bind(bayesianChartBtns));
        bayesianChartBtns.appendChild(btn)

    })
    function showChart(e) {
        Array.from(this.children).forEach(el => el.classList.remove("selected"))
