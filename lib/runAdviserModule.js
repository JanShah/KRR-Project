...
function demandForecast(olderOrders, recentOrders) {
    ...
    const combined = { ...olderOrders, ...recentOrders };
...
function adviserModule(worker, { prior, future, boxes }) {
...
function analyseProposedOrder(proposedOrders, boxes) {
    ...
    const totalSpace = 180000000000;
    const availableSpace = totalSpace * (Number(document.getElementById("maxSpace").value) / 100 || 0.10); // 10% of 450m3
    ...
    for (const { item, quantity, itemVolume, itemCost } of demandData) {
        ...
        let adjustedPrice = itemCost * adjustedQuantity

        const costForOrder = adjustedPrice;
        let volumeForOrder = itemVolume * adjustedQuantity;
        while (volumeForOrder / oldVolume > volumeMultiplier) {
            //need to reduce to meet the space constraints
            adjustedQuantity = Math.round(quantity * volumeMultiplier)
