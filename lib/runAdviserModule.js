...
function demandForecast(olderOrders, currentOrders) {
    const predictionHorizon = document.getElementById("weeksOfCover").value || 12
    console.log("WEEKS OF COVER: ",predictionHorizon)
    ...
    const alpha = Number(document.getElementById("varianceTuning").value) || 1.4; // Variance tuning 
    const minVariance = 0;
    const varianceFloor = Number(document.getElementById('varianceFloor')) || 0;
    ...
    Object.keys(combined).forEach(key => {
        ...
        Object.keys(currentObservation).forEach(code => {
            ...
            const noise = Math.random() * (Number(document.getElementById("randomNoise").value) || 0.1); // Small random noise
    ...
    // Forward predictions
    for (let week = 1; week <= Number(predictionHorizon); week++) {
...
function proposePurchaseOrder(demandForecastData) {
    const noOfWeeks = parseInt(document.getElementById("weeksOfCover").value, 10) || 12;
    const pastWeeks = Object.keys(demandForecastData).sort((a, b) => a - b); // Sort weeks in ascending order
    ...
    const roundedOrders = Object.keys(futureOrders).map(key => {
        return {
            code: key,
            qty: Math.round(futureOrders[key])
        };
    });
...
function analyseProposedOrder(proposedOrders, boxes) {
    ...
    const availableSpace = 900000000000 * (Number(document.getElementById("maxSpace") / 100) || 0.10); // 10% of 450m3
    ...
    for (const { item, quantity, itemVolume, itemCost } of demandData) {
        ...
        const proportionalBudget = Math.ceil(budgetWeight * (availableBudget - (itemCost * quantity)));
        ...
        if (
            ...
            const volumeDifference = (itemVolume * adjustedQuantity) - (itemVolume * quantity);
            totalDemandVolume += volumeDifference;
            const valueDifference = (itemCost * adjustedQuantity) - (itemCost * quantity);
            totalDemandCost += valueDifference
        } else {
            // Further reduce quantity to fit within constraints
            const maxAffordableQuantity = Math.floor((availableBudget) / itemCost);
    ...
    const orderData = constrainedOrders.map(order => {
        ...
        if (totalCost > availableBudget) {
            ...
            totalCost = itemCost * adjustedQuantity; // Recalculate total cost after adjustment
        }
        ...
        return {
            ...
            "Space Used %": ((totalSpaceUsage * 100) / availableSpace).toFixed(2)
        };
    });
