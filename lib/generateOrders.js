 ...
 * @returns 
 */
function generateOrders(customers, boxes, suppliers, { startDate = "01/01/2023", endDate = "04/01/2023", startingBudget = 5000 } = settings) {
        ...
        if (dayIndex && dayIndex % 7 === 0) {
            ...
            salesOrders["delivered"] = salesOrders["delivered"].filter(order => {
                if (daysBetween(today, order.shippedDate) <= 14) {
                    if (order.satisfactionRating < 3 && Math.random() > 0.8) {
...
function getCategories() {
    const categories = [
        new Category("Tools", 0.2, [5, 15], [0.3, 0.7], [20, 150], [5, 10, 12, 24]),
        new Category("Books", 0.3, [3, 7], [0.2, 0.5], [1, 10], [1, 5, 20]),
        new Category("Gifts", 0.7, [4, 17], [0.3, 1.2], [5, 75], [6, 10, 12, 24]),
        new Category("Fashion", 0.6, [5, 20], [0.5, 0.9], [10, 50], [1, 3, 6, 12]),
        new Category("Electronics", 0.7, [15, 100], [0.1, 0.2], [75, 500], [2, 5]),
    ];
    return categories;
}
