...
function generateOrders(customers, boxes, suppliers, { startDate = "01/01/2023", endDate = "04/01/2023", startingBudget = 5000, packagingLeadTime = 5, useAdviser = false, preorder = undefined } = settings) {
    ...
    let maxBudgetForPackaging = 500;
    let randomNoiseForPackaging = 0.1
    let minVariance = 0;
    let maxSpaceForPackaging = .5
    if (preorder) {
        backOrderBoxes = preorder.reduce((acc, cur) => {
            acc[cur.item] = cur.quantity;
            return acc;
        }, {})
        reOrderTimeHorizon = Number(preorder.timeHorizon) || 6;
        maxBudgetForPackaging = preorder.budget || 500
        maxSpaceForPackaging = preorder.maxSpace || .5
        minVariance = preorder.minVariance
        debugger
    }
    ...
    while (today < endingDate) {
        ...
        if (reOrderTimeHorizon > -1) {
            if (dayIndex && dayIndex % (7 * reOrderTimeHorizon) === 0) {
                ...
                debugger
                const [recommendation, analysis] = analyseProposedOrder(proposedOrder, boxesObject, maxBudgetForPackaging, maxSpaceForPackaging);
                const boxOrderFromPrediction = recommendation.reduce((acc, cur) => {
                    acc[cur.item] = cur.quantity * boxesObject[cur.item].pack
                    return acc
                }, {})

                const boxOrder = createBoxOrder(boxOrderFromPrediction, packagingSupplier, today, boxesObject, purchaseOrders);
                purchaseOrders.pending.push(boxOrder);
