async function generateOrders(products, customers, boxData, suppliers) {
    console.error("This function should not be used"); return;
    const time = new Date()
...
function genOrders(customers, boxes, suppliers) {
    ...
    const endingDate = new Date("6/01/2023");
    ...
    while (today < endingDate) {
        ...
        const newCustomerCount = Math.min(20, Math.floor(customers.length / daysTillEnd));
        ...
        salesOrders["paid"] = salesOrders["paid"].filter(order => {
            ...
            return processOrder(order, inventoryList, boxes, stockShortage, boxDemand, today, salesOrders)
        })
